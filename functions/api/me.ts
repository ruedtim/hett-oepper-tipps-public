 ...
 */
export const onRequestGet: PagesFunction<Env, string, RequestData> = async ({ data, env }) =>
  json({
    name: data.user.name,
    ...
    // Der Signal-Link reist bewusst über diese Antwort statt als Konstante im
    // Bundle: Das Bundle ist vor dem Gate abrufbar, der Chat der Runde geht
    // Fremde nichts an — und Gäste ebenso wenig (geteiltes Passwort, keine
    // Person dahinter).
    signalChat: data.user.isGuest ? null : (env.SIGNAL_CHAT_URL ?? null),
